...
###   bitcoincloudcore.org and bitcoincloud.org and compares them.
...
WORKINGDIR="/tmp/bitcoincloud_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://bitcoincloudcore.org"
HOST2="https://bitcoincloud.org"
BASEDIR="/bin/"
VERSIONPREFIX="bitcoincloud-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitcoincloud-',
...
if [ $? -ne 0 ]; then
   echo "bitcoincloud.org failed to provide signature file, but bitcoincloudcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bitcoincloud.org and bitcoincloudcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the BitcoinCloud Core binary release signing key installed?"
   fi
