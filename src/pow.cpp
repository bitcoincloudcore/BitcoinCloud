...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader* pblock, const Consensus::Params& params)
{
    ...
    if (pindexLast->nHeight >= FORK_BLOCK &&
        ...
        // forked yet
        if (!params.postForkPowLimit.IsNull() && params.postForkPowLimit != params.powLimit) {
...
bool CheckProofOfWork(uint256 hash, unsigned int nBits, const Consensus::Params& params, uint256* best)
{
    ...
    // Check range
    if (fNegative || bnTarget == 0 || fOverflow || bnTarget > UintToArith256(params.postForkPowLimit.IsNull() ? params.powLimit : params.postForkPowLimit)) {
        return false;
    }
