...
const char* const BITCOINCLOUD_CONF_FILENAME = "bitcoincloud.conf";
const char* const BITCOINCLOUD_PID_FILENAME = "bitcoincloudd.pid";
...
public:
    ...
    static inline std::pair<bool, std::string> GetArg(const ArgsManager& am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "bitcoincloudd -foo=bar
...
#else
    const char* pszModule = "bitcoincloud";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BitcoinCloud
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BitcoinCloud
    // Mac: ~/Library/Application Support/BitcoinCloud
    // Unix: ~/.bitcoincloud
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitcoinCloud";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BitcoinCloud";
#else
    // Unix
    return pathRet / ".bitcoincloud";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", BITCOINCLOUD_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", BITCOINCLOUD_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure BitcoinCloud Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("BitcoinCloud Core") == std::string::npos) {
